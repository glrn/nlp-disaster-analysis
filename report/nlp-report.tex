...
\begin{itemize}[noitemsep, nolistsep]
	...
	\item\textbf{Chicago} - 2,092 tweets posted in the \textit{Chicago, IL} area during 28 Aug. - 3 Sept., containing at least one of the words [\tweet{car, plane, fire, dead}]
\end{itemize}

For all datasets we avoided from "re-tweeted" tweets (\tweet{RT}), to prevent repetitions of popular tweets.\\
...
The most popular entities among the \textbf{Chicago} tweets:

\begin{itemize}[noitemsep]
	\item\textbf{Our NER} - \todo{Omri}
	\item\textbf{Ritter et al.} - \tweet{Twitter, Chicago Fire, Tuna Fire, Chicago Fire MLS, Soccer, Buzcast, Chicago, LA, South Side, Hammond, Montreal}
\end{itemize}

The comparison starts to fail when comparing the uncommon named entities, which show that  Ritter's works better. However, our method is sufficient for understanding the trivial details of large-scale disasters - location and type of disaster.
