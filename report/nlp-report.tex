...
We aim to extract relevant information from tweets we previously classified as disaster-related. Named-entity recognition (NER) allows us to extract names of people, organizations, locations and time expressions. Ritter et al., 2011 \cite{Ritter11} provide information and methods on how to extract named entities out of tweet corpora, by using a Twitter-specific POS tagset (similarly to the tagset we used in our previous classifiers). In this section, we present our attempt to use regular database, which was trained on news and articles, and extract named entities out of tweets.

\paragraph{Groningen Meaning Ban (GMB)}
To develop a named-entity recognizer we used the Groningen Meaning Bank (GMB\endnote{\textbf{"Groningen Meaning Bank" Dataset by University of Groningen}: comprises of thousands of texts in raw and tokenized formats, tags for part of speech, named entities and lexical categories, and discourse representation structures compatible with first-order logic. \url{http://gmb.let.rug.nl/}}) - a large corpus containing thousands of texts in raw and tokenized formats, POS tags, named entities by lexical categories and much more. The top level categories of the corpus are:
...
We used only the top-level categories, which are sufficient for extracting relevant information on disasters. As the database contains POS tagging in NTLK format, we used the Twitter POS tagger by Gimpel et al. \cite{POS-Tagging} and converted the coarse-grained tags to the closet NLTK POS tag available.\\
...
\todo{Any ideas of what should we write here?}
