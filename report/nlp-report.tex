...
The first mission is \textit{identification} of disaster-related tweets in a varied corpus. We implemented several classifiers, the best of which achieved \todo{\%} accuracy on the dataset.

The second missions is binary classification of disaster-related tweets to one of two categories, \textit{subjective} tweets vs. \textit{objective} reports on disasters. To achieve this we manually tagged 2,410 disaster-related tweets. The rationale behind this task is that objective tweets (for example, informative news reports) are likely to be published after the event had already become clear to emergency services, while subjective tweets may contain invaluable first-person testimonies.
...
\todo{this is a framework that could work on other problems (political tweets)...}
